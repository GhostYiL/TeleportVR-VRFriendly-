...
namespace Ruby.ButtonAPI
{
    ...
    public class ButtonAPI : VRModule
    {
        ...
        public IEnumerator Start()
        {
            ...
            ButtonAPI.Q_RUBYLogo = new Ruby.ButtonAPI.QMSingleButton(ButtonAPI.Q_Functions, 1, 0, "Teleport", delegate ()
            {
                Teleport.TELEPORT();
            }, "Teleport to a player", new Color?(Color.cyan), new Color?(Color.white));
           
            ButtonAPI.Q_RUBYLogo = new Ruby.ButtonAPI.QMSingleButton(ButtonAPI.Q_Functions, 2, 0, "Save Current Position", delegate ()
            {
                MainModule.GetTeleport().SavePosition();
                //if (Event.current.control && Input.GetKeyDown(KeyCode.PageUp));
                MiscUtils.GetVRCUiManager().QueueHudMessage("Saved\nCurrent Position");
            }, "Save Current Position", new Color?(Color.cyan), new Color?(Color.white));

            ButtonAPI.Q_RUBYLogo = new Ruby.ButtonAPI.QMSingleButton(ButtonAPI.Q_Functions, 3, 0, "TP_To Saved Position", delegate ()
            {
                MainModule.GetTeleport().TPToStoredPos();
                //if (Event.current.control && Input.GetKeyDown(KeyCode.PageDown));
                MiscUtils.GetVRCUiManager().QueueHudMessage("Teleporting:\n To Stored Position");
            }, "Teleport To Saved Postion", new Color?(Color.cyan), new Color?(Color.white));
            /*
            Things what im thinking about:
